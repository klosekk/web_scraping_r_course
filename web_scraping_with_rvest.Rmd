...
- The star * performs as a wildcard operator. This means that inserting * into a path such as \/body\/div\/*\/ul leads to identifying information that starts at body, moves along to div and then takes all tags that contain an ul-tag.
- The vertical bar | allows us to select multiple paths in one query, e.g. "\/\/p | \/\/a"


Further, XPATH can be used with conditional statements (predicative). In this case

XPATH also the ability to create very precise paths by specifying unique relationships between nodes. This is outside the scope of this module. If you are interested, please have a look at w3schools explanation of [XPATH Axes](https://www.w3schools.com/xml/xpath_axes.asp). For instance, you can specify that a node \<p\> must be preceded by a tag such as \<span\> at some point in the tree structure (ancestor-relationship).
